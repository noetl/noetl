...
# - System tools (Docker, kubectl, helm, kind, task, psql, pyenv, tfenv, uv)
...
#
# After bootstrap completes, you can use 'task' commands.
# Usage:
...
install_tools_macos() {
    ...
    # Handle go-task installation separately (may need tap switch)
    if command_exists task; then
        log_info "task already installed"
    else
        log_info "Installing go-task/tap/go-task..."
        # Check if task is installed from wrong tap
        if brew list go-task >/dev/null 2>&1; then
            log_warning "go-task installed from homebrew/core, switching to go-task/tap..."
...
setup_cluster() {
    ...
    # Run full bootstrap
    log_info "Running NoETL bootstrap (this may take 5-10 minutes)..."
    task noetl:k8s:bootstrap
    
    log_success "NoETL infrastructure ready"
    log_info "NoETL UI: http://localhost:8083"
    log_info "To access Grafana: kubectl port-forward -n vmstack svc/vmstack-grafana 3000:80"
    log_info "To access Postgres: kubectl port-forward -n postgres svc/postgres 5432:5432"
}

# Create project taskfile if it doesn't exist
create_project_taskfile() {
    local taskfile="$PROJECT_ROOT/Taskfile.yml"
    
    if [[ -f "$taskfile" ]]; then
        log_info "Taskfile.yml already exists in project root"
        return 0
    fi
    
    log_info "Creating Taskfile.yml in project root..."
    
    cat > "$taskfile" << 'EOF'
version: '3.45'

# Import all NoETL tasks
includes:
  noetl:
...
tasks:
  ...
  dev:setup:
    ...
    cmds:
      ...
      - task: noetl:noetl:k8s:bootstrap
  ...
  bootstrap:verify:
    ...
    cmds:
      - |
        ...
        command -v kind && echo "✓ kind" || echo "✗ kind"
        command -v jq && echo "✓ jq" || echo "✗ jq"
        command -v yq && echo "✓ yq" || echo "✗ yq"
        command -v task && echo "✓ task" || echo "✗ task"
        command -v psql && echo "✓ psql" || echo "✗ psql"
        [[ -d .venv ]] && echo "✓ .venv exists" || echo "✗ .venv missing"
        [[ -f .venv/bin/noetl ]] && echo "✓ noetl CLI" || echo "✗ noetl CLI missing"

  # Example: Register your playbooks
  playbooks:register:
    desc: Register project playbooks with NoETL
    cmds:
      - |
        if [[ -d playbooks ]]; then
          for playbook in playbooks/*.yaml; do
            ...
            .venv/bin/noetl register "$playbook" --host localhost --port 8083
          done
        else
...
EOF
    ...
    log_success "Taskfile.yml created in $PROJECT_ROOT"
}
...
```bash
# Using NoETL CLI
noetl register credentials/my_cred.yaml --host localhost --port 8083
...
main() {
    ...
    echo "  2. List tasks:       task --list"
    echo "  3. Access NoETL:     http://localhost:8083"
    echo "  4. View logs:        kubectl logs -n noetl -l app=noetl-server -f"
    echo ""
    echo "Next Steps:"
    echo "  - Add your playbooks to playbooks/"
    echo "  - Add your credentials to credentials/"
    echo "  - Register: .venv/bin/noetl register playbooks/my_workflow.yaml"
    echo "  - Execute: .venv/bin/noetl execute playbook my_workflow"
    echo ""
    echo "Documentation:"
    echo "  - Bootstrap guide: noetl/ci/bootstrap/README.md"
    echo "  - NoETL docs: https://noetl.io"
    echo ""
}
