...
**Important**: do **NOT** use a step-level `when` attribute anymore. All step-level decision logic must be expressed via `case`.
...
In the DSL spec (`dsl_spec.md`):

* Keep `next` as described:
...
* **Do NOT** support `when/then/else` on `next` in the schema anymore.

* Make clear: `next` is unconditional; conditional flows go through `case`.

### **2.2 `next` as an action in `then`, with `args`**

Under `case[*].then`, support a **`next` action** of this shape:
...
**Do not use `with` here.** `with` is reserved (for now) for step-level inputs (e.g. workbook/python steps), not for cross-step transitions.

### **2.3 Rewrite `weather_loop_example.yaml` transitions**

In `weather_loop_example.yaml` you currently have patterns like:
...
`- step: start`  
  `desc: "Start Weather Analysis Workflow"`  
  `# optional structural next (can keep as simple default)`  
  `next: city_loop`  
  `case:`  
...
And for transitions that currently use `with` on `next`:
...
And in subsequent steps that branch based on `evaluate_weather_directly.alert`, replace old conditional `next.when/then` blocks with `case`\-based transitions and `next.args`.

### **2.4 `with` vs `args`**

* Keep `with` **only** where the step type definition expects it (e.g., `workbook`, `python`, `playbooks`) as inputs to that step/task.

* Use **`args`** exclusively for cross-step parameter passing in **`next` actions**.
...
Inside `case[*].then`, support (reusing existing semantics where possible):
...
No need to invent new semantics, just make all of these usable inside `then`.

---

## **4\. Server-side DSL control-flow engine**

Create/refactor a **control-flow engine** module on the server, e.g. `dsl/engine.py`:
...
## **5\. Workers: read commands, send events (no queue writes)**
