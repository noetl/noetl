...
function HttpNode({ id, data }: NodeProps<Node<HttpData>>) {
    ...
    return (
        <div style={{ padding: 8, border: '1px solid #1890ff', borderRadius: 8, fontSize: 12, background: '#fff', width: 240, cursor: 'pointer' }} onDoubleClick={openEditor}>
            ...
            <div style={{ fontSize: 11, wordBreak: 'break-all', lineHeight: 1.3 }}>{summaryUrl || <span style={{ opacity: 0.5 }}>(no url)</span>}</div>
            ...
            <Modal open={modalOpen} onCancel={() => setModalOpen(false)} title={data?.name ? `HTTP Config: ${data?.name}` : 'HTTP Config'} width={640} footer={[<Button key="cancel" onClick={() => setModalOpen(false)}>Cancel</Button>, <Button key="save" type="primary" onClick={commit} disabled={!!headerError}>Save</Button>]}> <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                <div style={{ display: 'flex', gap: 8 }}>
                    <Select value={draft.method} onChange={(v) => setDraft(d => ({ ...d, method: v }))} style={{ width: 120 }} options={['GET', 'POST', 'PUT', 'PATCH', 'DELETE', 'HEAD', 'OPTIONS'].map(m => ({ value: m, label: m }))} />
                ...
                <div>
                    ...
                    <Input.TextArea value={headerInput} rows={4} placeholder='{"Authorization":"Bearer ..."}' onChange={e => handleHeaderChange(e.target.value)} style={{ fontFamily: 'monospace' }} />
                ...
                <div>
                    ...
                    <Input.TextArea value={draft.body} rows={5} placeholder='{"key":"value"} or raw text' onChange={e => setDraft(d => ({ ...d, body: e.target.value }))} style={{ fontFamily: 'monospace' }} />
                </div>
                <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                    <Input type="number" value={draft.timeout} placeholder="timeout ms" onChange={e => setDraft(d => ({ ...d, timeout: e.target.value }))} style={{ width: 140 }} />
