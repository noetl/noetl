...
def test_parse_content():
    """Test the parsing logic with the actual weather example file"""
    ...
    for i, line in enumerate(lines):
        ...
        if i < 50:  # Show first 50 lines for debugging
            print(f"Line {i:2d}: '{line}' (trimmed: '{trimmed}', indent: {leading_spaces})")
        ...
        if in_section:
            ...
            elif trimmed.startswith('desc:') or trimmed.startswith('description:'):
                # Extract description and use it to improve task name if name is generic
                import re
                desc_match = re.search(r'(?:desc|description):\s*[\'\"](.*?)[\'\"]|(?:desc|description):\s*([^\'\"]+)', trimmed)
                if desc_match and current_task and (current_task['name'].startswith('Task ') or not current_task['name']):
                    description = (desc_match.group(1) or desc_match.group(2) or '').strip()
                    # Remove surrounding quotes
                    description = description.strip('\'"')
                    current_task['name'] = description
                    print(f"✓ Updated task name from description: {current_task['name']}")
