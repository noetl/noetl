#!/bin/bash
# Build NoETL package for PyPI distribution
...
echo -e "${BLUE}Building NoETL Package for PyPI...${NC}"
...
with open('pyproject.toml', 'r') as f:
    ...
    if 'fastapi' in content:
        ...
        print('FastAPI dependency not found')
"
...
if ! python3 -c "import twine" &> /dev/null; then
    echo -e "${YELLOW}  Installing twine for validation...${NC}"
    pip install twine
fi
