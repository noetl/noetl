...
function show_usage {
    ...
    echo "  --no-postgres       Skip building Postgres image"
...
echo -e "${YELLOW}NoETL Docker Image Build Script${NC}"
echo "This script will build Docker images for NoETL deployments."
echo
...
if $BUILD_POSTGRES; then
    ...
    BUILD_END_TS=$(date +%s)
    echo -e "${GREEN}Postgres build finished in $((BUILD_END_TS - BUILD_START_TS))s.${NC}"
    
    if $PUSH_IMAGES && [ -n "$REGISTRY" ]; then
        echo -e "${GREEN}Pushing Postgres image to registry...${NC}"
        docker push "${REGISTRY}postgres-noetl:${TAG}"
    fi
else
    echo -e "${YELLOW}Skipping Postgres image build as requested.${NC}"
fi
...
echo -e "  1. Update the deployment YAML files to use these images"
echo -e "  2. Apply the updated deployment files with kubectl"
echo
