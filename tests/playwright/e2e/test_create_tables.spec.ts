...
test.describe('Create Tables', () => {
    ...
    test.beforeAll(() => {
        console.log('Registering create_tables...');
        execSync('noetl register tests/fixtures/playbooks/save_storage_test/create_tables.yaml --host localhost --port 8082', { stdio: 'inherit' });
    });
    ...
    test('should open catalog page', async ({ page }) => {
        ...
        // Locate the first element that contains the text "create_tables"
        const exampleItem = page.locator("(//*[text()='create_tables']/following::button[normalize-space()='Execute'])[1]");
        ...
        // Assertions
        await expect(rowData.Playbook).toBe('create_tables');
        ...
        // await expect(updatedRowData.Playbook).toBe('create_tables');

        // Click the "View" button for the "create_tables" task
        // TODO fix the selector below from "Unknown" to "create_tables"
