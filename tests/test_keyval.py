...
class TestKeyVal(unittest.TestCase):
    ...
    def test_base64_value(self):
        ...
        self.assertEqual(kv.base64_value("workflow_base64"), base64.b64encode("test".encode()).decode())
        with self.assertRaises(ValueError):
    ...
    def test_base64_str(self):
        self.assertEqual(KeyVal.str_to_base64("test"), base64.b64encode("test".encode()).decode())

    def test_base64_to_yaml(self):
        yaml_str = base64.b64encode(json.dumps({"a": 1}).encode()).decode()
        self.assertEqual(KeyVal.base64_to_yaml(yaml_str), {"a": 1})

    def test_from_json(self):
        json_str = json.dumps({"a": 1})
        kv = KeyVal(json.loads(json_str))
