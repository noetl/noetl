...
case class WebserviceConf(
  ...
  override def printMessage =
    s"WebserviceConf call ${this.displayName} - curl ${this.httpMethod} ${this.url}"
}
...
case class ShellConf(
  ...
  override def printMessage =
    s"ShellConf call for ${this.displayName} - sh ${this.shellScript} ${this.scriptParams}"
}
...
case class JdbcConf(
    ...
    with ActionJdbcBase {
  override def printMessage =
    s"JdbcConf call for ${this.displayName} - jdbc ${this.databaseUrl} ${this.queryParams} ${this.queryString}"
}
...
case class SshConf(
    ...
    with ActionSshBase {
  override def printMessage =
    s"SshConf call for ${this.displayName} ssh ${this.sshHost} ${this.sshPort} ${this.sshUser}"
}
...
case class ScpConf(
    ...
    with ActionScpBase {
  override def printMessage =
    s"ScpConf call for ${this.displayName} ssh ${this.sourceHost} ${this.sourcePort} ${this.sourceUser}"
}
