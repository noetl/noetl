...
trait ActionConfig {
  val name: String
  def actionType(config: Config): ActionType =
    ActionConfig.getActionType(config)
  ...
  def next(config: Config): List[String] =
    config.getStringList("next").asScala.toList
  def actionRun(config: Config): ActionRun =
    ActionRun(Try(Some(config.getConfig("run"))).getOrElse(None))
...
object ActionConfig {
  ...
  private case class Webservice(name: String, config: Config)
  ...
  def apply(actionName: String, config: Config): ActionConfig = config match {
    ...
    case action if this.getActionType(action) == ACTION =>
      Action(actionName, config)
    case start if this.getActionType(start) == START =>
      Start(actionName, config)
    ...
    case webservice if this.getActionType(webservice) == WEBSERVICE =>
      Webservice(actionName, config)
    case shell if this.getActionType(shell) == SHELL =>
      Shell(actionName, config)
