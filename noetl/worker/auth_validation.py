...
def validate_auth_single(
    auth_config: Union[str, Dict[str, Any]], context: str = ""
) -> List[str]:
    """
    ...
    if source not in VALID_AUTH_SOURCES:
        errors.append(
            f"{prefix}invalid source '{source}', must be one of {VALID_AUTH_SOURCES}"
        )
    ...
    if inject is not None:
        if not isinstance(inject, dict):
            ...
            if kind and kind not in ["http_header"]:
                errors.append(
                    f"{prefix}inject.kind '{kind}' not supported (supported: http_header)"
                )
            ...
            if kind == "http_header":
                ...
                if not header or not isinstance(header, str):
                    errors.append(
                        f"{prefix}inject.kind=http_header requires 'header' string"
                    )
...
def validate_auth_multi(auth_config: Dict[str, Any], context: str = "") -> List[str]:
    ...
    for alias, auth_spec in auth_config.items():
        ...
        alias_errors = validate_auth_single(
            auth_spec, f"{context}auth.{alias}" if context else f"auth.{alias}"
        )
...
def validate_auth_for_plugin(
    plugin_type: str, auth_config: Any, context: str = ""
) -> List[str]:
    """
    ...
    if expected_arity == "single" and actual_arity == "multi":
        errors.append(
            f"{prefix}{plugin_type} steps require single auth (string or single auth object), got multi-auth (alias map)"
        )
        return errors
    ...
    if expected_arity == "multi" and actual_arity == "single":
        ...
        if plugin_type == "duckdb":
            logger.debug(
                f"DuckDB step using single auth - will auto-wrap to multi-auth"
            )
        else:
            errors.append(
                f"{prefix}{plugin_type} steps require multi-auth (alias map), got single auth"
            )
            return errors
...
def validate_step_auth(step: Dict[str, Any]) -> List[str]:
    ...
    if not isinstance(tool_name, str) or not tool_name.strip():
        errors.append(
            f"step '{step_name}' is missing required 'tool' field for auth validation"
        )
        return errors
    ...
    if isinstance(task, dict):
        ...
        if not isinstance(task_tool, str) or not task_tool.strip():
            errors.append(
                f"step '{step_name}' task configuration missing required 'tool'"
            )
            task_type = ""
        else:
            task_type = task_tool.strip().lower()
