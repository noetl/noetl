...
class ExecutionRequest(BaseModel):
    ...
    **Lookup Strategies** (priority order):
    ...
    3. `playbook_id`: Legacy identifier (treated as path, auto-resolves version)
    ...
    **MCP Compatibility**:
    ...
    At least one identifier (catalog_id, path, or playbook_id) must be provided.
    """
    ...
    playbook_id: Optional[str] = Field(
        default=None,
        description="Legacy playbook identifier (alias for path, backward compatible)",
        example="examples/weather/forecast"
    )
    version: Optional[str] = Field(
        default="latest",
        description="Version identifier (semantic version or 'latest'). Used with path/playbook_id lookup",
        example="v1.0.0"
    )
    ...
    type: ExecutionType = Field(
        default="playbook",
        description="Type of execution: playbook, tool, model, or workflow",
        alias="execution_type"
    )
    parameters: Optional[Dict[str, Any]] = Field(
        default=None,
        description="Input parameters/payload for execution",
        alias="input_payload"
    )
    ...
    def validate_and_normalize(self):
        ...
        if not self.catalog_id and not self.path and not self.playbook_id:
            raise ValueError(
                "At least one identifier must be provided: catalog_id, path, or playbook_id"
            )
        
        # Normalize: if playbook_id is provided but not path, use playbook_id as path
        if self.playbook_id and not self.path:
            self.path = self.playbook_id
        
        # Default version to 'latest' if using path-based lookup
        if (self.path or self.playbook_id) and not self.catalog_id:
...
class ExecutionResponse(BaseModel):
    ...
    playbook_id: Optional[str] = Field(
        None,
        description="Playbook identifier (for backward compatibility)"
    )
