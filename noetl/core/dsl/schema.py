...
class DatabaseSchema:
    ...
    def mark_error_resolved(self, error_event_id: int, resolution_notes: str = None) -> bool:
        ...
        try:
            ...
            with self.conn.cursor() as cursor:
                cursor.execute(f"""
                    INSERT INTO {self.noetl_schema}.event (execution_id, event_id, created_at, event_type, status, meta)
    ...
    def get_errors(self, 
        ...
        try:
            ...
            query = f"SELECT execution_id, event_id, created_at, node_id, node_name, error, meta, stack_trace FROM {self.noetl_schema}.event WHERE event_type = 'error'"
            params = []
            if error_type:
